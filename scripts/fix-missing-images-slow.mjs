...
async function main() {
  ...
  console.log(`Generando imágenes para ${rows.length} proyectos (con delay de 10s)...\n`);
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (imgUrl) {
      ...
      console.log(`  ⚠ Fallo — reintentando en 15s...`);
    ...
    // 10 segundos entre cada imagen para evitar rate limit
    if (i < rows.length - 1) await new Promise(r => setTimeout(r, 10000));
  }
